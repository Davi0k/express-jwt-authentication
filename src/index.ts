...
export default function(options: IOptions): (req: Request, res: Response, next: NextFunction) => void {
    if (options.secret == undefined)
        if (process.env.EXPRESS_JWT_AUTHENTICATION_SECRET)
            options.secret = (process.env.EXPRESS_JWT_AUTHENTICATION_SECRET as jwt.Secret);
        else throw new Error("Property `secret` must be set either in IOptions parameter or in process.env.");

    if (options.algorithm == undefined)
        if (process.env.EXPRESS_JWT_AUTHENTICATION_ALGORITHM)
            options.algorithm = (process.env.EXPRESS_JWT_AUTHENTICATION_ALGORITHM as jwt.Algorithm);
